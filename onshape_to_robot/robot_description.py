...
class RobotURDF(RobotDescription):
    ...
    def addDummyLink(self, name, visualMatrix=None, visualSTL=None, visualColor=None):
        ...
        # XXX: We use a low mass because PyBullet consider mass 0 as world
        # fixed
        if self.noDynamics:
...
class RobotSDF(RobotDescription):
    ...
    def addJoint(
        ...
        self.append(
            '<limit><effort>%g</effort><velocity>%g</velocity>%s</limit>' %
            (self.jointMaxEffortFor(name),
             self.jointMaxVelocityFor(name),
             lowerUpperLimits))
        self.append('</axis>')
